# Larger LAINR Decoder Implementation
# Increases capacity for better super-resolution
...
class SharedTokenCrossAttention(nn.Module):
    """Cross-attention with spatial bias (unchanged)"""
...
class LAINRDecoderLarge(nn.Module):
    """
    LARGER LAINR Decoder for better super-resolution

    Key changes from original:
    1. hidden_dim: 256 → 512 (2x width)
    2. num_layers: 1 → 3 (3x depth per scale)
    3. Residual connections between layers
    4. Total parameters: ~450K → ~4M
    """

    def __init__(self, feature_dim=64, input_dim=2, output_dim=3,
                 sigma_q=16, sigma_ls=[128, 32], n_patches=256,
                 hidden_dim=512,  # CHANGED: 256 → 512
                 context_dim=256,
                 learnable_frequencies=True,
                 num_layers=3):  # NEW: Number of layers per scale
        super().__init__()
    ...
    def forward(self, coords_decoding, tokens, coords_modulation=None):
        ...
        for k in range(self.layer_num):
            ...
            h_l = self.bandwidth_lins[k](x_l)  # Now goes through multiple layers
...
if __name__ == "__main__":
    ...
    print("="*70)
    print("DECODER SIZE COMPARISON")
    print("="*70)

    # Original small decoder
    from gaussian_fourier_lainr import LAINRDecoderGaussian

    decoder_small = LAINRDecoderGaussian(
        feature_dim=64,
        sigma_q=16,
        sigma_ls=[128, 32],
        n_patches=256,
    ...
    # New large decoder
    decoder_large = LAINRDecoderLarge(
        feature_dim=64,
        sigma_q=16,
        sigma_ls=[128, 32],
        n_patches=256,
        ...
        context_dim=256,
        learnable_frequencies=True,
        num_layers=3  # 3x deeper
    )

    params_small = count_parameters(decoder_small)
    params_large = count_parameters(decoder_large)

    print(f"\nSmall decoder (original):")
    print(f"  hidden_dim: 256")
    print(f"  num_layers: 1 per scale")
    print(f"  Parameters: {params_small:,}")

    print(f"\nLarge decoder (new):")
    print(f"  hidden_dim: 512")
    print(f"  num_layers: 3 per scale")
    print(f"  Parameters: {params_large:,}")

    print(f"\nIncrease: {params_large / params_small:.1f}x")

    print("\n" + "="*70)
    print("Expected benefits:")
    print("  - Better texture synthesis capability")
    print("  - More complex feature combinations")
    print("  - Higher capacity for learning high-freq patterns")
    print("  - Expected +3-5 dB PSNR improvement at 128×128")
    print("="*70)
