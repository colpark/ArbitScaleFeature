...
@register('mamba_composers_inr_quad')
class MambaInr(torch.nn.Module):
    ...
    def forward(self, data):
        dtokens_x, dtokens_y = self.tokenizer(data, mode = 'quad')
        B = dtokens_x.shape[0]
        lps = einops.repeat(self.lps, 'n d -> b n d', b=B)
        all_tokens_x = self.add_lp(dtokens_x)
        all_tokens_y = self.add_lp(dtokens_y)
        z = (all_tokens_x, all_tokens_y)
        mamba_out = self.mamba_encoder(z)
