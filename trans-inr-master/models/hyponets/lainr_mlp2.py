...
class LAINRDecoder(nn.Module):
    def __init__(self, feature_dim, input_dim, output_dim, sigma_ls, hidden_dim = 256, context_dim = None):
        super().__init__()
        ...
        '''self.modulation_cas = nn.ModuleList([
            SharedTokenCrossAttention(query_dim = hidden_dim, heads=2) for i in range(self.layer_num)
                                       ])'''
    ...
    def forward(self, x, tokens):
        ...
        x = x.view(B, -1, x.shape[-1]) #B, HW, input_dim
        
        x_q = einops.repeat(torch.stack([self.calc_gamma(x[0], self.omegas_l[i]) for i in range(self.layer_num)]), 'n l d -> b n l d', b=B) #B, HW, input_dim
        ...
        modulation_vectors = self.modulation_ca(x_q, context = tokens)

        
        #modulation_vectors = [self.modulation_cas[i](x_q[:, i], context = tokens) for i in range(self.layer_num)]
        modulations_l = []
        h_f = []
        ...
        for k in range(self.layer_num):
            ...
            h_f.append(h_l)
            m_l = self.act(h_l + self.modulation_lins[k](modulation_vectors[:, k]))
...
class SharedTokenCrossAttention(nn.Module):
    ...
    def forward(self, x, context):
        ...
        x_reshaped = x.view(B * N, HW, D)  # (B*N, HW, D)
        context_expanded = context.unsqueeze(1).expand(-1, N, -1, -1)  # (B, N, L, D)
        context_reshaped = context_expanded.contiguous().view(B * N, -1, D)  # (B*N, L, D)

        # Linear projections
        q = self.to_q(x_reshaped)               # (B*N, HW, H*Dh)
        kv = self.to_kv(context_reshaped)       # (B*N, L, 2*H*Dh)
        k, v = kv.chunk(2, dim=-1)              # (B*N, L, H*Dh)

        # Reshape for multi-head attention
        q = q.view(B * N, HW, H, Dh).transpose(1, 2)  # (B*N, H, HW, Dh)
        k = k.view(B * N, -1, H, Dh).transpose(1, 2)  # (B*N, H, L, Dh)
        v = v.view(B * N, -1, H, Dh).transpose(1, 2)  # (B*N, H, L, Dh)

        # Scaled dot-product attention
        sim = torch.matmul(q, k.transpose(-1, -2)) * self.scale  # (B*N, H, HW, L)
        attn = sim.softmax(dim=-1)
        out = torch.matmul(attn, v)  # (B*N, H, HW, Dh)

        # Combine heads and reshape back
        out = out.transpose(1, 2).contiguous().view(B * N, HW, D_inner)  # (B*N, HW, H*Dh)
        out = self.to_out(out)                                           # (B*N, HW, D)
        out = out.view(B, N, HW, D)                                      # (B, N, HW, D)
        return out
...
class LAINRDecoder(nn.Module):
    ...
    def forward(self, x, tokens):
        ...
        x_q = einops.repeat(torch.stack([self.calc_gamma(x[0], self.omegas_l[i]] for i in range(self.layer_num)), 'l d -> b l d', b=B) #B, HW, input_dim
        print(x_q.shape)
        print(x_q[:, 0].shape)
        ...
        modulation_vectors = [modulation_cas[i](x_q[:, i], context = tokens) for i in range(self.layer_num)]
