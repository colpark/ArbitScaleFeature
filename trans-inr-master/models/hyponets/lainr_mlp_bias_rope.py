...
class SharedTokenCrossAttention(nn.Module):
    def __init__(self, query_dim, context_dim=None, heads=2, dim_head=64):
        ...
        self.inv_freq = self.get_rotary_angles(dim_head) 

    def get_rotary_angles(self, dim, max_position=2048, base=10000):
        """
        Generates the frequency vector for rotary embedding (like in GPTNeoX or LLaMA).
        Returns a tensor of shape (dim//2,)
        """
        inv_freq = 1.0 / (base ** (torch.arange(0, dim, 2).float() / dim))
        return inv_freq  # shape: (dim // 2,)
    
    def apply_rope(self, x, pos, inv_freq):
        """
        Applies RoPE to tensor x with positions pos.
        x: shape (batch, seq_len, dim)
        pos: shape (seq_len,) or (batch, seq_len)
        inv_freq: shape (dim//2,)
    
        Returns: rotated x of same shape
        """
        # Shape: (seq_len, dim//2)
        angles = pos[:, None] * inv_freq[None, :]  # broadcast over dims
        sin = angles.sin()
        cos = angles.cos()
    
        # Split last dim into pairs
        x1, x2 = x[..., ::2].to(x.device), x[..., 1::2]
    ...
    def forward(self, x, context, bias=None):
        ...
        pos_q = torch.linspace(0, 1, HW) * HW
        pos_q = pos_q.to(x.device)
        pos_k = torch.linspace(0, 1, L) * HW
        pos_k = pos_k.to(x.device)
        inv_freq = self.inv_freq.to(x.device)
        q_rope = self.apply_rope(q, pos_q, inv_freq)  # (B, T, D)
        k_rope = self.apply_rope(k, pos_k, inv_freq)
