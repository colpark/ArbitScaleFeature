...
@register('lainr_mlp_bias_gff')
class LAINRDecoder(nn.Module):
    def __init__(self, feature_dim, input_dim, output_dim, sigma_q, sigma_ls, n_patches, hidden_dim = 256, context_dim = None, alpha = 1.0):
        super().__init__()
        ...
        #self.omegas = self.sample_1_over_f(self.n, f_min=1.0, f_max=sigma_q, alpha=alpha)
        ...
        #self.omegas_l = [torch.rand(self.n)*sigma_ls[i] for i in range(self.layer_num)]
             # Example replacement for self.omegas_l

        self.omegas_l = [self.sample_1_over_f(self.n, f_min=1.0, f_max=sigma_ls[i], alpha = alpha)
    ...
    def sample_1_over_f(self, n, f_min, f_max, alpha=1.0):
        u = torch.rand(n)
        if alpha == 1.0:
            # log-uniform (1/f)
            return torch.exp(u * (math.log(f_max) - math.log(f_min)) + math.log(f_min))
        else:
