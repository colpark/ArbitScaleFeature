...
@register('lainr_mlp_bias_init')
class LAINRDecoder(nn.Module):
    def __init__(self, feature_dim, input_dim, output_dim, sigma_q, sigma_ls, n_patches, hidden_dim = 256, H = 32, W = 32, context_dim = None):
        super().__init__()
        ...
        coord = make_coord_grid((H, W), (0, 1))
        self.coord = coord.view(-1, coord.shape[-1])
        indexes = self.get_patch_index(self.coord, self.patch_num, self.patch_num)
        rel_distances = self.approximate_relative_distances(indexes, self.patch_num, self.patch_num, n_patches)
        bias_init = rel_distances.transpose(1, 0).clone().detach()
        self.bias =  nn.Parameter(bias_init) # (L, HW)
    ...
    def forward(self, x, tokens):
        ...
        #(B, HW, 1, D)
        #(B, HW, L, D)
        #bias =  einops.repeat(self.bias, 'l n -> b l n', b=B).to(x.device) #B, L, HW
        bias =  einops.repeat(self.bias, 'l n -> b l n', b=B)
