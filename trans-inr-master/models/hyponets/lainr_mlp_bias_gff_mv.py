...
@register('lainr_mlp_bias_gff_mv')
class LAINRDecoder(nn.Module):
    def __init__(self, feature_dim, input_dim, output_dim, sigma_ls, n_patches, hidden_dim = 256, context_dim = None):
        super().__init__()
        self.layer_num = len(sigma_ls)
        self.n = feature_dim//(2*input_dim)
        #self.omegas = torch.logspace(1, math.log10(sigma_q), self.n)
        #self.omegas = torch.rand(self.n)*sigma_q
        ...
        self.omegas_l = [torch.rand(self.n)*self.sigma_ls[i] for i in range(self.layer_num)]

        self.query_lin = nn.Linear(feature_dim, hidden_dim)

        #self.modulation_ca = PerPixelCrossAttention(query_dim = hidden_dim, heads=2)
        self.modulation_cas = nn.ModuleList([SharedTokenCrossAttention(query_dim = hidden_dim, heads=2) for i in range(self.layer_num)])
        ...
        self.hv_lins = nn.ModuleList([
            nn.Linear(hidden_dim, hidden_dim) for _ in range(len(self.sigma_ls) - 1)
        ])

        self.out_lins = nn.ModuleList([
            nn.Linear(hidden_dim, output_dim) for _ in range(len(self.sigma_ls))
        ])
        self.act = nn.ReLU()
    ...
    def forward(self, x, tokens):
        ...
        modulation_vectors = [self.modulation_cas[i](x_q[:, i], context = tokens, bias = bias) for i in range(self.layer_num)]
        modulations_l = []
        h_f = []
