...
@register('mamba_patch_tokenizer_fmri')
class MambaPatchTokenizer(nn.Module):

    def __init__(self, input_size, patch_size, dim, padding=0, img_channels=1, pos_emb = 'learned', pos_emb_dim = 32):
        super().__init__()
        ...
        if (self.learned_posemb):
            self.posemb = nn.Parameter(torch.randn(self.n_patches, dim))
        else:
        ...
        self.prefc = nn.Linear(patch_size[0] * patch_size[1] * img_channels, self.dim)
    ...
    def forward(self, data):
        ...
        x = self.prefc(x) #B, L, D

        #before prefc, 
        if (self.learned_posemb):
            x = x + self.posemb.unsqueeze(0)
        else:
            x = torch.cat((self.posemb.repeat(B, 1, 1).to(x.device), x), dim = 2) #B, L, D+4L
            #x = self.postfc(x)
