...
class BaseTrainer():
    ...
    def train_epoch(self):
        ...
        t1 = time.time()
        step_idx = 0
        log_every = 1000  # Customize this as needed
        ...
        for data in pbar:
            ...
            if self.is_master:
                pbar.set_description(f'train: loss={ret["loss"]:6f} grad_norm={ret["grad_norm"]:.7f}')
    
                if step_idx % log_every == 0 and 'grad_norm' in ret and ret['grad_norm'] > 1e-1:
                    tqdm.write(f"[Epoch {self.epoch} | Step {step_idx}] grad_norm={ret['grad_norm']:.4e}")
                    if self.enable_tb:
                        self.writer.add_scalar('train/grad_norm_step', ret['grad_norm'], global_step=self.epoch * 10000 + step_idx)
                    if self.enable_wandb:
                        wandb.log({'train/grad_norm_step': ret['grad_norm']}, step=self.epoch * 10000 + step_idx)
        ...
        # ✅ Only log final average once per epoch
        logtext = 'train:'
    ...
    '''def train_epoch(self):
        ...
        t1 = time.time()
        step_idx = 0
        log_every = 200  # Customize this as you like
        ...
        for data in pbar:
            ...
            if self.is_master:
                pbar.set_description(f'train: loss={ret["loss"]:.4f}')
        
                # ✅ Clean gradient norm logging
                if step_idx % log_every == 0 and 'grad_norm' in ret and ret['grad_norm'] > 1e-6:
                    tqdm.write(f"[Epoch {self.epoch} | Step {step_idx}] grad_norm={ret['grad_norm']:.4e}")
        
                    # Optional: Log to TB / W&B without printing
                    if self.enable_tb:
                        self.writer.add_scalar('train/grad_norm_step', ret['grad_norm'], global_step=self.epoch * 10000 + step_idx)
                    if self.enable_wandb:
                        wandb.log({'train/grad_norm_step': ret['grad_norm']}, step=self.epoch * 10000 + step_idx)
