...
@register('fmri_trainer_lainr_ddp_resize_lr_batched')
class ImgrecTrainer(BaseTrainer):
    ...
    def _iter_step(self, data, is_train, p=1.0, k=32):
        data = {k: (v.cuda() if isinstance(v, torch.Tensor) else v) for k, v in data.items()}
        gt_full = data.pop('fmri_sequence')
        
        resize = self.cfg['resize']
        gt_full = self.transform_sequence(gt_full, resize, is_train)
        ...
        # Clean up coord tensor to save memory
        del coord
        ...
        # Clean up gt tensor to save memory
        del gt
        torch.cuda.empty_cache()
        ...
        indices = torch.randperm(N, device=gt_flat.device)[:sample_size]  # (sample_size,)
        coord_sampled = coord_flat[:, indices, :]  # (B, sample_size, dim)
        gt_sampled = gt_flat[:, indices, :]       # (B, sample_size, C)
        
        # Clean up intermediate tensors
        del coord_flat, gt_flat, indices
        torch.cuda.empty_cache()
        
        # Split coordinates into k batches
        batch_size = coord_sampled.shape[1]
        chunk_size = (batch_size + k - 1) // k  # Ceiling division to ensure all coords are processed
        ...
        with torch.autocast(device_type="cuda", enabled=self.cfg['use_amp'], dtype=torch.bfloat16):
            for i in range(k):
                ...
                # Get batch slice
                coord_batch = coord_sampled[:, start_idx:end_idx, :]  # (B, chunk_size, dim)
                gt_batch = gt_sampled[:, start_idx:end_idx, :]        # (B, chunk_size, C)
                
                # Forward pass for this batch
                pred_batch = self.model_ddp(gt_full, coord_batch)
                
                # Store results (keep gradients for training)
                all_preds.append(pred_batch)
                all_gts.append(gt_batch)
                
                # Clean up batch coordinate tensor (but keep pred_batch and gt_batch for gradients)
                del coord_batch
                del pred_batch
                del gt_batch
                torch.cuda.empty_cache()
        
        # Clean up sampled tensors
        del coord_sampled, gt_sampled
        torch.cuda.empty_cache()
        
        # Concatenate all predictions and ground truths
        pred = torch.cat(all_preds, dim=1)  # Concatenate along coordinate dimension
        gt_final = torch.cat(all_gts, dim=1)
        ...
        mses = ((pred - gt_final)**2).view(B, -1).mean(dim=-1)
        loss = mses.mean()
        psnr = (-10 * torch.log10(mses)).mean()
        
        # Clean up final tensors
        del pred, gt_final, mses
        torch.cuda.empty_cache()
