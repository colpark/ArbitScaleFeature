...
class BaseTrainer():
    def __init__(self, cfg): # changed - No longer needs rank passed in
        ...
        if self.distributed:
            ...
            self.log(f'Distributed training enabled on {self.world_size} GPUs.') # change end [init distributed]
    ...
    def make_datasets(self):
        ...
        if fmri_data_cfg is not None:
            ...
            self.log(f"Initializing DataModule from config: {data_cfg_path}")
            data_module = DataModule(config_path=data_cfg_path)
            data_module.setup()
